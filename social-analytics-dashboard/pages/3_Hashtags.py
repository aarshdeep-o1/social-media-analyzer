...
filtered["hashtag_range"] = pd.cut(filtered["hashtags_count"], bins=[0,5,10,15,20,30], labels=["1-5","6-10","11-15","16-20","21-30"])
summary = filtered.groupby("hashtag_range", observed=True).agg(avg_engagement=("engagement_rate","mean"), avg_likes=("likes","mean"), total_posts=("post_id","count")).reset_index()
...
st.dataframe(filtered[["post_id","upload_date","media_type","hashtags_count","engagement_rate","likes","content_category"]].sort_values("engagement_rate", ascending=False).reset_index(drop=True), use_container_width=True, hide_index=True)
